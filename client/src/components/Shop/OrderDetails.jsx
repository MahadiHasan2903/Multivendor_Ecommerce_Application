...
import {
  transporterAssignedOrderStatus,
  getAllOrdersOfShop,
...
import { getTransporters } from "../../redux/actions/serviceProviderAction";
...
const OrderDetails = () => {
  ...
  const transporterRows = serviceProviders.map((transporter) => ({
    id: transporter._id,
    name: transporter.name,
    email: transporter.email,
    phone: transporter.phoneNumber,
  ...
  return (
    <div className="flex flex-col justify-between mx-5 800px:flex-row">
      ...
      <div className="flex flex-col w-full 800px:w-[45%] m-4 mt-10 items-center">
        {data &&
          (data.assignedToTransporter.length !== 0 ? (
            <h1 className="text-2xl font-bold text-center">
              You&apos;ve already assigned this order to a transporter
            </h1>
          ) : (
            <div className="flex flex-col items-center w-[80%] m-4 mt-10 800px:w-full">
              ...
              <div className="w-full ml-5">
                <DataGrid rows={transporterRows} columns={columns} />
              </div>
            </div>
          ))}
      </div>
